...
const createCard = async (req, res) => {
  try {
    const { title, content, columnId, link, author, status } = req.body;
    ...
    // Créer la carte
    const card = new Card({ title, content, columnId, link, author, status });
    await card.save();
...
const updateCard = async (req, res) => {
  try {
    const { id } = req.params;
    const { content, title, link, columnId, author, status } = req.body;
